...
import { ContactPanelsPanel } from './contact-panels-panel'
...
export function OverviewDashboard() {
  ...
  const resolveEnabled = (key: string, fallback: boolean) => {
  ...
  const contactPanels = useMemo(() => {
    const panels: Array<{ title?: string; image: string; caption?: string }> = []
  ...
  const overviewLayout = useMemo(
    () =>
      normalizeOverviewLayout(
        typeof status?.overview_layout === 'string'
          ? status.overview_layout
          : ''
      ),
    [status?.overview_layout]
  )

  const leftPanelsVisible =
    apiInfoEnabled || announcementsEnabled || faqEnabled || contactPanels.length > 0

  const orderedMainPanels = useMemo(() => {
    const panels = {
      'api-info': apiInfoEnabled ? (
        <CardStaggerItem key='api-info'>
          <ApiInfoPanel />
        </CardStaggerItem>
      ) : null,
      announcements: announcementsEnabled ? (
        <CardStaggerItem key='announcements'>
          <AnnouncementsPanel />
        </CardStaggerItem>
      ) : null,
      'contact-panels':
        contactPanels.length > 0 ? (
          <CardStaggerItem key='contact-panels'>
            <ContactPanelsPanel panels={contactPanels} />
          </CardStaggerItem>
        ) : null,
      faq: faqEnabled ? (
        <CardStaggerItem key='faq'>
          <FAQPanel />
        </CardStaggerItem>
      ) : null,
    } as const
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      {(leftPanelsVisible || uptimeEnabled) && (
        <CardStaggerContainer
          className={cn(
