...
export function SubscriptionPlansCard({
  ...
  const isMemberUpgradeMode = mode === 'member-upgrade'
  ...
  const enableStripe = !!topupInfo?.enable_stripe_topup || !!status?.enable_stripe_topup
  const enableCreem = !!topupInfo?.enable_creem_topup
  const enableOnlineTopUp =
    !!topupInfo?.enable_online_topup || !!status?.enable_online_topup
  ...
  const displayPlans = useMemo(
    ...
    [isMemberUpgradeMode, plans]
  )
  const isAvailable = loading || displayPlans.length > 0 || hasAny
  ...
  if (displayPlans.length === 0 && !hasAny && !isMemberUpgradeMode) {
    return null
  }
  ...
  return (
    <>
      <TitledCard
        title={
          isMemberUpgradeMode ? t('Upgrade Plans') : t('Subscription Plans')
        }
        description={
          isMemberUpgradeMode
            ? t('Only plans marked for the member upgrade page are shown here')
          ...
          {displayPlans.length > 0 ? (
            <div className='grid grid-cols-1 gap-3 2xl:grid-cols-2 2xl:gap-4'>
              {displayPlans.map((p, index) => {
                ...
                const isPopular = index === 0 && displayPlans.length > 1
            ...
            <p className='text-muted-foreground py-4 text-center text-sm'>
              {isMemberUpgradeMode
                ? t('No member upgrade plans available')
