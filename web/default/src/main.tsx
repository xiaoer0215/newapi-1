...
import '@/lib/dayjs'
import { applyFaviconToDom, applySeoToDom } from '@/lib/dom-utils'
...
;(function initSystemBranding() {
  try {
    ...
    const apply = (name: string, subtitle?: string) => {
      const title = [name, subtitle].filter(Boolean).join(' - ') || name
    ...
    try {
      ...
      if (saved) {
        ...
        if (s?.system_name) apply(s.system_name, s.system_subtitle)
        applySeoToDom({
    ...
    getStatus()
      .then((s) => {
        if (s?.system_name) {
          apply(s.system_name as string, s.system_subtitle as string | undefined)
          try {
        ...
        applySeoToDom({
          description: s?.seo_description as string | undefined,
          keywords: s?.seo_keywords as string | undefined,
        })
