 ...
 * invalid, or otherwise unusable 鈥?the caller treats null as "do not narrow",
...
function isNavItemVisible(
  ...
  // Handle dynamic chat presets type 鈥?also runs the admin 脳 user AND gate
...
/**
 * Filter sidebar navigation groups by admin 脳 user sidebar_modules config.
 *
 * Two layers, AND-combined:
 *   1. Admin (status.SidebarModulesAdmin) 鈥?authoritative, falls back to
 *      DEFAULT_SIDEBAR_MODULES when empty/invalid. Disabling here hides the
 *      item for everyone regardless of user preference.
 *   2. User (auth.user.sidebar_modules) 鈥?narrower overlay, null sentinel
...
export function useSidebarConfig(navGroups: NavGroup[]): NavGroup[] {
  ...
  const userConfig = useMemo(() => {
    ...
    // (e.g. root accounts), skip the user overlay entirely 鈥?a stale
