...
const NoticeModal = ({
  ...
  const latestAnnouncement = processedAnnouncements[0] || null;
  ...
  const renderLatestSystemAnnouncement = () => {
    ...
    return (
      <div className='max-h-[55vh] overflow-y-auto pr-2 card-content-scroll'>
        <div
          className={latestAnnouncement.isUnread ? 'shine-text' : ''}
          dangerouslySetInnerHTML={{ __html: htmlContent }}
        />
        {htmlExtra && (
          <div
      ...
      footer={
        <div className='flex justify-end'>
          ...
          <Button type='primary' onClick={onClose}>
            {latestOnly ? t('我知道了') : t('关闭公告')}
          </Button>
        </div>
      }
      size={isMobile ? 'full-width' : 'large'}
    >
      {latestOnly ? renderLatestSystemAnnouncement() : renderBody()}
    </Modal>
  );
};
