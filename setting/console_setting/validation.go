...
func ValidateConsoleSettings(settingsStr string, settingType string) error {
	...
	case "TopNoticeItems":
		return validateTopNoticeItems(settingsStr)
	case "FAQ":
...
func validateTopNoticeItems(topNoticeItemsStr string) error {
	list, err := parseJSONArray(topNoticeItemsStr, "??????")
	if err != nil {
		return err
	}
	if len(list) > 20 {
		return fmt.Errorf("???????????????20??")
	}

	validTopNoticeTypes := map[string]bool{
		"info":    true,
		"notice":  true,
		"warning": true,
	}
	...
	for i, item := range list {
		...
		if len(content) > 240 {
			return fmt.Errorf("??d????????????????????40???", i+1)
		}
		if err := checkDangerousContent(content, i+1, "??????"); err != nil {
			return err
		}

		if typeAny, exists := item["type"]; exists {
			typeStr, ok := typeAny.(string)
			if !ok || strings.TrimSpace(typeStr) == "" {
				return fmt.Errorf("top notice #%d has invalid type", i+1)
			}
			if !validTopNoticeTypes[typeStr] {
				return fmt.Errorf("top notice #%d has unsupported type %q", i+1, typeStr)
			}
		}

		if routesAny, exists := item["routes"]; exists {
			routes, ok := routesAny.([]interface{})
			if !ok {
				return fmt.Errorf("top notice #%d routes must be an array", i+1)
			}
			if len(routes) > 20 {
				return fmt.Errorf("top notice #%d has too many routes", i+1)
			}
			for _, routeAny := range routes {
				route, ok := routeAny.(string)
				if !ok {
					return fmt.Errorf("top notice #%d has an invalid route entry", i+1)
				}
				route = strings.TrimSpace(route)
				if route == "" {
					return fmt.Errorf("top notice #%d has an empty route", i+1)
				}
				if route != "/" && !strings.HasPrefix(route, "/") {
					return fmt.Errorf("top notice #%d route must start with /", i+1)
				}
				if len(route) > 120 {
					return fmt.Errorf("top notice #%d route is too long", i+1)
				}
			}
		}

		startAtAny, exists := item["startAt"]
		if !exists {
			return fmt.Errorf("??d????????????????????", i+1)
		}
		startAtStr, ok := startAtAny.(string)
		if !ok || strings.TrimSpace(startAtStr) == "" {
			return fmt.Errorf("??d??????????????????????", i+1)
		}
		...
		if endAtAny, exists := item["endAt"]; exists {
			endAtStr, ok := endAtAny.(string)
			if !ok {
				return fmt.Errorf("??d?????????????????????", i+1)
			}
			if strings.TrimSpace(endAtStr) != "" {
...
func GetTopNoticeItems() []map[string]interface{} {
	return getJSONList(GetConsoleSetting().TopNoticeItems)
}
